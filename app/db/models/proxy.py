...
class InboundHost(Base):
    ...
    # ML-KEM-768 + X25519 hybrid credentials for VLESS post-quantum encryption.
    # mlkem_public_key  -> client `encryption` string used in subscription URLs:
    #   mlkem768x25519plus.<mode>.<rtt>.<x25519_pub>.<mlkem_client_eK>  (~1620 chars)
    # mlkem_private_key -> server `decryption` string used in xray inbounds:
    #   mlkem768x25519plus.<mode>.<ticket>.<x25519_priv>.<mlkem_seed>  (~135 chars)
    # Generated by `app.utils.mlkem.generate_mlkem_bundle`.
    ...
    mlkem_public_key = Column(String(4096))
