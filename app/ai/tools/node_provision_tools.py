...
async def onboard_node_from_donor(
    ...
    if not step1["ok"]:
        ...
        return {
            ...
            "hint": (
                "Both nodes must be present in the panel's "
                "in-memory NodeRegistry before cloning. Missing "
                f"node(s): {missing_ids}. The fix is panel-side "
                "and needs no SSH: call `enable_node(node_id)` "
                "for EACH missing node — it forces a fresh "
                "`add_node` and re-registers the gRPC client "
                "with the current cert/key. Donor failures are "
                "not a reason to ask the admin for SSH on the "
                "donor host. Wait ~15 s after each call and "
                "retry the macro. If `enable_node` itself "
                "returns an error, that error IS the original "
                "cause of the missing entry (cert load failure, "
                "address parse error, etc.) — report it "
                "verbatim and fix the underlying issue before "
                "retrying. Only switch to the "
                "`diagnose-node-down` skill with SSH probes "
                "when `enable_node` succeeded but a follow-up "
                "`get_node_recent_errors` shows real "
                "`kind=sync`/`kind=ssl` failures (not "
                "`node_not_loaded`)."
            ),
        }
