...
@register_tool(
    ...
    description=(
        ...
        "REMARK EDITING — read this before passing `remark`: NEVER type the remark "
        "value from your own output (your tokenizer drops emoji variation selectors "
        "and ZWJ; the saved string will look identical but compare differently). "
        "Always (a) call `get_host_info(host_id)` first and use the returned `remark` "
        "verbatim as the base, (b) compute the new value with literal substring "
        "replacement on that base, (c) pass it back unchanged. For multi-host "
        "renames (e.g. 'replace Франция with FR everywhere') use "
        "`preview_remark_replace` + `bulk_replace_in_remarks` instead — they perform "
        "the substring edit in the database without round-tripping the string "
        "through your output. Only fields whose values differ from the sentinel "
        "are updated:\n"
